...
- Chrome MV3 插件：采集 Seller Center 当前页商品并幂等导入本地 SQLite；领取任务、打开机会页、填写商品 ID、人工确认结果；CSS 选择器可配置。
...
2. 服务端重新读取商品详情，不信任浏览器传入的标题、类目或品牌数据。
3. 使用叶子类目 ID 分别查询 `PRODUCT`、`KEYWORD`、`CATEGORY` 机会，并读取候选机会详情。
...
4. 打开插件设置页，填写本地服务地址、`EXTENSION_SHARED_KEY`、本地店铺 ID；采集选择器留空时使用内置通用规则。
5. 打开 Seller Center 商品列表页，在插件中点击“读取当前页”，确认预览后点击“导入本地服务”。相同店铺和 Product ID 会更新快照，不会重复新增。
6. 从管理页把任务显式切到 `extension`，再由插件领取。
...
- Chrome 商品采集提供通用 DOM 规则和可配置选择器；真实 MX Seller Center 页面仍需校准，页面改版后可能需要更新选择器。
